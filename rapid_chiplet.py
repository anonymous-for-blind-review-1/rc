...
def compute_power_summary(chiplets, placement, packaging, link_lengths_internal):
	...
	# Compute power of links
	total_link_power = sum([packaging["link_power"] if packaging["link_power_type"] == "constant" else eval(packaging["link_power"])(link_length) for link_length in link_lengths_internal.values()])
	# Compute total interposer area
...
def compute_ici_latency(technology, chiplets, placement, packaging, ici_graph, ici_routing, link_lengths_internal):
	(c, r, n, neighbors, relay_map, nodes_by_type) = ici_graph
	...
	edge_latencies = [{nei : (packaging["link_latency"] if packaging["link_latency_type"] == "constant" else int(math.ceil(eval(packaging["link_latency"])(link_lengths_internal[(i,nei)])))) for nei in neighbors[i]} for i in range(n)]
	# Iterate through chiplets
	...
	for traffic in ["C2C","C2M","C2I","M2I"]:
		if len(paths_per_class[traffic]) > 0:
...
def compute_metrics(design, compute_area, compute_power, compute_link, compute_cost, compute_latency, compute_throughput, compute_thermal, routing):
	# Results
	results = {}
	# Timing
	timing = {}
	...
	compute_link = compute_link or compute_latency or compute_power
	# Technology node
	technology = None
	...
	# Compute the power summary
	if compute_power:
		power_summary = compute_power_summary(chiplets, placement, packaging, link_lengths_internal)
...
if __name__ == "__main__":
	...
	# Compute metrics
	results = compute_metrics(design = design, 
	...
	# Store results
	hlp.write_file("./results/%s.json" % args.results_file, results)
