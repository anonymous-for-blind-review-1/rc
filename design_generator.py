...
def generate_sparse_hamming_graph(rows, cols, SR, SC, chiplets, chiplet_name, phy_map):
	# Load chiplet
	...
	placement = {"chiplets" : [], "interposer_routers" : []}
	topology = []
	# Add the mesh-links to the other links	
	(SR, SC)  = ([1] + SR, [1] + SC)
	# Points to the next available PHY for horizontal and vertical links
	next_phy_h = [0 for i in range(rows * cols)]
	next_phy_v = [0 for i in range(rows * cols)]
	# Add compute chiplets and links between them
	for row in range(rows):
		for col in range(cols):
			# Chiplet id
			cid = row * cols + col	
			# Compute and set chiplet position
			(x,y) = (col * w, row * h)
			placement["chiplets"].append({"position" : {"x" : x, "y" : y}, "rotation" : 0, "name" : chiplet_name})
			# Create Horizontal links
			for ll in SR:
				if col + ll < cols:
					# Id of the other chiplet
					ocid = row * cols + (col + ll)
					# Attach link to this chiplet	
					ep1 = {"type" : "chiplet", "outer_id" : cid, "inner_id" : phy_map["H" + str(next_phy_h[cid])]}
					# Attach link to the other chiplet
					ep2 = {"type" : "chiplet", "outer_id" : ocid, "inner_id" : phy_map["H" + str(next_phy_h[ocid])]}
			...
			for ll in SC:
				if row + ll < rows:
					# Id of the other chiplet
					ocid = (row + ll) * cols + col
					# Attach link to this chiplet	
					ep1 = {"type" : "chiplet", "outer_id" : cid, "inner_id" : phy_map["V" + str(next_phy_v[cid])]}
					# Attach link to the other chiplet
					ep2 = {"type" : "chiplet", "outer_id" : ocid, "inner_id" : phy_map["V" + str(next_phy_v[ocid])]}
					# Add link to topology
					topology.append({"ep1" : ep1,"ep2" : ep2})
					# Update next PHY
					next_phy_v[cid] += 1
